C  19 Feb 2016: Modified version (Joe Vallino)
C  Added ability to pass rpar and ipar in SOLOUT routine
C  Search for "jjv" to locate edits
C -----------------------------------------------------------------------------------
...
C
C      subroutine solout(m,t,y,f,k,ord,irtrn,rpar,ipar)   
...
c      rpar    real(8) vector passed by user
c      ipar    integer vector passed by user
C      ................
C      return
C      end
C
...
C      double precision t(k),y(m,k),f(m,k),rpar(*)
C      integer i,j
...
C
      ...
      real(8) t0vec(1) ! jjv 26 sep 08 Added t0vec(1) to prevent compiler errors is SOLOUT
C
C OUTPUT PARAMETERS
C
...
c jjv NJ0 is never initialized, so do so here.  I supposed it doesn't matter. (6 aug 2011)
c jjv IT is also never initialized!
c jjv qinf not initialized. Don't have any guess.  
c      nj0 = 0d0   ! note sure about these, so commenting out my additions 24 mar 2016
c      it = 0
c      qinf = .false.
...
c jjv compiler indicates h0 never initialized??? so set it to h here
      h0 = h 
      ...
      IF (IERR.NE.0) THEN 
c jjv comment out write statements          
c          WRITE(6,1020) IERR
c 1020     FORMAT(/,/,'ERROR CODE IERR = ', I3, /
c     &               'RETURNED BY THE SUBROUTINE FCN',/,/)
c          WRITE(6,900) T0
          IDID = -6
          GOTO 800
      END IF
      ...
      ! jjv 26 Sep 08: Changed two statements below, and added
      ! newstatments below that to prevent compiler errors.
      ! since T0 is scaler, but dummary variable in SOLOUT is a vector,
      ! it can cause an error. 
C      IF(IOUT.EQ.1)
C     &  CALL SOLOUT(M,T0,Y0,F0,1,ORD,IRTRN)
      
C     19 Feb 2016: jjv,  added rpar and ipar to SOLOUT            
      IF(IOUT.EQ.1) then
        t0vec(1) = t0  
        CALL SOLOUT(M,t0vec,Y0,F0,1,ORD,IRTRN,rpar,ipar)
      end if
...
C NUMERICAL JACOBIAN
               DO I=1,M
                 ...
                 IF (IERR.NE.0) THEN
c jjv comment out write statement                    
c                     WRITE(6,1030) IERR
c 1030                FORMAT(/,/,'ERROR CODE IERR = ', I3, /,
c     &               'RETURNED BY THE SUBROUTINE FCN DURING',/,
c     &               'THE NUMERICAL EVALUATION OF THE JACOBIAN',/,/)
c                    WRITE(6,900) T0
                    GOTO 800
                 END IF
                 IF (JBAND) THEN
...
170      CONTINUE
         ...
         IF (INFO.NE.0) THEN
               NSING = NSING + 1
               IF (NSING.GT.5) THEN
c jjv comment out write                  
c                  WRITE(*,*) 'MATRIX IS REPEATEDLY SINGULAR, IER= ',INFO
c                  WRITE(6,900) T0
                  IDID = -4
                  GOTO 800
               ELSE
...
c jjv comment out write                     
c                    WRITE(6,*) 'STEPSIZE TOO SMALL, H=',H
c                    WRITE(6,900) T0
                    IDID = -3
                    GOTO 800
                  END IF
...
c jjv removed write statement            
c             WRITE(*,*) 'TOO MANY CONSECUTIVE NEWTON FAILURES'
c             WRITE(6,900) T0
             IDID = -5
             GOTO 800
             RETURN
...
C     19 Feb 2016: jjv,  added rpar and ipar to SOLOUT      
      IF (IOUT.EQ.1)
     &   CALL SOLOUT(M,T,Y,F,K,ORD,IRTRN,rpar,ipar)
...
c jjv comment out write            
c            WRITE(6,*) 'MORE THAN NMAX = ',MAXSTEP, ' STEPS ARE NEEDED'
c            WRITE(6,900) T0
            IDID = -2
            GOTO 800
         END IF
